...
class Topic(db.Model):
    ...
    def all_articles(self, limit=2000):
        ...
        if hasattr(Topic, 'cached_articles') and (self.cached_articles.get(self.id, None)):
            zeeguu_core.log_n_print (f"Topic: getting the cached articles for topic: {self.title}")
        ...
        zeeguu_core.log_n_print("computing and caching the articles for topic: " + self.title)
