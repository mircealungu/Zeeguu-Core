...
class UserReadingSessionTest(ModelTestMixIn, TestCase):
    ...
    def test__update_reading_session_scenario1(self):
        ...
        new_read_session =  UserReadingSession.update_reading_session(db_session, 
                                                            event, 
                                                            self.read_session.user_id, 
                                                            self.read_session.article_id
                                                        )
        assert new_read_session != self.read_session
    ...
    def test__update_reading_session_scenario4(self):
        ...
        assert (new_read_session == self.read_session) and (self.read_session.is_active == False)
    ...
    def test__find_by_user(self):
        ...
        active_sessions = UserReadingSession.find_by_user(user.id, self.LONG_TIME_AGO, self.LONG_TIME_AFTER, True)
        assert active_sessions

    def test__find_by_cohort(self):
        cohort_id = self.read_session.user.cohort_id
        active_sessions = UserReadingSession.find_by_cohort(cohort_id, self.LONG_TIME_AGO, self.LONG_TIME_AFTER,
                                                           True)
        assert active_sessions
    ...
    def test__find_by_article_scenario1(self):
        ...
        cohort_id = self.read_session.user.cohort_id
        active_sessions = UserReadingSession.find_by_article(article_id, self.LONG_TIME_AGO, self.LONG_TIME_AFTER,
                                                            True, cohort_id)
        assert active_sessions
